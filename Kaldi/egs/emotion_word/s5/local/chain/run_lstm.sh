...
use_gpu=wait
gmm=tri5a
build_tree_ali_dir=exp/tri5a_ali  # used to make a new tree for chain topology, should match train data
label_delay=8
...
dir=exp/chain${nnet3_affix}/lstm${tdnn_affix}_sp
...
echo " ##########9999999999999999999999#################33"
if [ $stage -le 9 ]; then
...
echo "######1212121212121212121212########"
if [ $stage -le 12 ]; then
  rm -rf ./exp/chain/lstm7b_sp/egs/
  mkdir ./exp/chain/lstm7b_sp/egs/
...
cp -R /data/untitled/kaldi/egs/voxforge/s5/exp/nnet3--generate-alignments/ /data/untitled/kaldi/egs/voxforge/s5/exp/nnet3/
echo "#############131313131313131313131313########"
if [ $stage -le 13 ]; then
...
decode_suff=
echo "#####1515151515151515####"
if [ $stage -le 15 ]; then
  iter_opts=
  if [ ! -z $decode_iter ]; then
    iter_opts=" --iter $decode_iter "
  fi
  for decode_set in train test; do
      (

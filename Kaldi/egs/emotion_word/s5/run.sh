...
python3 local/pyaudio_recoder.py
for n in test; do
    local/voxforge_select.sh --dialect $dialects \
    /data/untitled/online_wav /data/untitled/online_wav/selected

done

StartTime=$(date +%s)
online_selected_data=/data/untitled/online_wav/selected

# Initial normalization of the data
local/online_data_prep.sh ${online_selected_data} || exit 1


# Prepare G.fst and data/{train,test} directories
local/online_format_data.sh || exit 1

dir=${DATA_ROOT}/mfcc
for x in online/test; do
 steps/make_mfcc.sh --cmd "$train_cmd" --nj $njobs \
   data/$x exp/make_mfcc/$x $mfccdir || exit 1;
 steps/compute_cmvn_stats.sh data/$x exp/make_mfcc/$x $mfccdir || exit 1;
done
...
steps/online/nnet3/prepare_online_decoding.sh data/lang_chain exp/nnet3/extractor exp/chain/tdnn7b_sp exp/chain/tdnn7b_sp/online
steps/online/nnet3/my_decode.sh --per-utt true --online true exp/chain/tdnn7b_sp/graph data/online/test exp/chain/tdnn7b_sp/onlinedecode 
...
utt=$(cat /data/untitled/kaldi/egs/online/s5/exp/chain/tdnn7b_sp/decode/per_utt/utt2spk.1)

iconv -f euc-kr -t utf-8 /data/untitled/kaldi/egs/online/s5/exp/chain/tdnn7b_sp/onlinedecode/scoring/log/best_path.7.log > x.txt
#iconv -f euc-kr -t utf-8 /data/untitled/kaldi/egs/online/s5/exp/chain/tdnn7b_sp/decode/scoring/log/best_path.7.log > x.txt
utt=$(echo ${utt} | cut -d' ' -f1)
#echo ${utt}
cat x.txt|grep "${utt}"

cat x.txt|grep "${utt}" > /data/untitled/a.txt
